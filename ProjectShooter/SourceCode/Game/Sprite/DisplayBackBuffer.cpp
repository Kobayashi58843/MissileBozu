#include "DisplayBackBuffer.h"

#include "..\\Direct3D.h"

//シェーダファイル名(ディレクトリも含む)
const char SHADER_NAME[] = "Shader\\DisplayBackBuffer.hlsl";

DisplayBackBuffer::DisplayBackBuffer(const float fWidth, const float fHeight)
...
//シェーダを作成する.
HRESULT DisplayBackBuffer::CreateShader()
{
...
//板ポリ(四角形)の頂点を作成.
void DisplayBackBuffer::CreateVertex()
{
	//表示スプライト幅、高さ.
...
//ワールド変換.
void DisplayBackBuffer::ConversionWorld(D3DMATRIX &mWorld)
{
	D3DXMATRIX mTrans;
...
void DisplayBackBuffer::InputConstantBuffer(const D3DMATRIX mWorld)
{
	...
	m_pDeviceContext11->PSSetShaderResources(1, 1, &m_pSRV);
}
