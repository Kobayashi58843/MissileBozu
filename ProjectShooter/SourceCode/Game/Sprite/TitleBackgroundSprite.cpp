...
//シェーダファイル名(ディレクトリも含む)
const char SHADER_NAME[] = "Shader\\TitleBackgroundSprite.hlsl";

TitleBackgroundSprite::TitleBackgroundSprite(const float fWidth, const float fHeight)
	: m_vSize({ fWidth, fHeight })
	, m_pMainSRV(nullptr)
	, m_pSubSRV(nullptr)
{
	CrearVECTOR2(m_vUV);
...
//シェーダを作成する.
HRESULT TitleBackgroundSprite::CreateShader()
{
...
//板ポリ(四角形)の頂点を作成.
void TitleBackgroundSprite::CreateVertex()
{
	//表示スプライト幅、高さ.
	float fW = (m_vSize.x) * m_fScale;
	float fH = (m_vSize.y) * m_fScale;

	float fU = (m_vSize.x) / m_vSize.x;
	float fV = (m_vSize.y) / m_vSize.y;
...
//ワールド変換.
void TitleBackgroundSprite::ConversionWorld(D3DMATRIX &mWorld)
{
	D3DXMATRIX mTrans;
...
void TitleBackgroundSprite::InputConstantBuffer(const D3DMATRIX mWorld)
{
	...
	m_pDeviceContext11->PSSetShaderResources(1, 1, &m_pMainSRV);
	m_pDeviceContext11->PSSetShaderResources(2, 1, &m_pSubSRV);
}
